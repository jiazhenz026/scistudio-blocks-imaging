...
``scistudio_blocks_imaging.io.load_image``: ``.tif``/``.tiff`` and
...
def test_t_img_002_module_importable() -> None:
    ...
    importlib.import_module("scistudio_blocks_imaging.io.load_image")
...
# ---------------------------------------------------------------------------
# Issue #1075: supported_extensions ClassVar declaration
# ---------------------------------------------------------------------------


class TestSupportedExtensionsClassVar:
    """LoadImage declares the canonical ``supported_extensions`` mapping
    (ADR-028 §D8 / #1075) and the module-level legacy constants
    (``_TIFF_EXTS`` / ``_ZARR_EXTS`` / ``_SUPPORTED_EXTS``) have been
    ...
    def test_module_level_legacy_constants_removed(self) -> None:
        """The pre-#1075 module-level constants are gone (ClassVar is the
        canonical source of truth)."""
        from scistudio_blocks_imaging.io import load_image

        assert not hasattr(load_image, "_TIFF_EXTS"), "_TIFF_EXTS must be removed per #1075"
        assert not hasattr(load_image, "_ZARR_EXTS"), "_ZARR_EXTS must be removed per #1075"
        assert not hasattr(load_image, "_SUPPORTED_EXTS"), "_SUPPORTED_EXTS must be removed per #1075"

    def test_detect_format_resolves_known_extensions(self, tmp_path: Path) -> None:
        """``_detect_format`` returns the registered identifier for each
        supported suffix and ``None`` for unknown suffixes."""
        block = LoadImage()
    ...
    def test_detect_format_case_insensitive(self, tmp_path: Path) -> None:
        """Suffix matching is case-insensitive per IOBlock._detect_format."""
        block = LoadImage()
        assert block._detect_format(tmp_path / "x.TIF") == "tiff"
        assert block._detect_format(tmp_path / "x.Tiff") == "tiff"

    def test_unsupported_extension_error_references_classvar(self, tmp_path: Path) -> None:
        """The ValueError on an unknown suffix references the sorted ClassVar keys."""
        bogus = tmp_path / "bogus.xyz"
        bogus.write_bytes(b"not an image")
        with pytest.raises(ValueError) as excinfo:
            LoadImage().load(BlockConfig(params={"path": str(bogus)}))
        msg = str(excinfo.value)
        # The error message includes the sorted ClassVar keys.
        assert ".tif" in msg
        assert ".tiff" in msg
        assert ".zarr" in msg

    def test_classvar_is_inherited_from_ioblock(self) -> None:
        """The ClassVar overrides the empty base default from IOBlock."""
        from scistudio.blocks.io.io_block import IOBlock

        assert IOBlock.supported_extensions == {}
        assert LoadImage.supported_extensions != IOBlock.supported_extensions

    def test_tiff_smoke_roundtrip_through_new_dispatch(self, tmp_path: Path) -> None:
        """End-to-end smoke: a .tif round-trips through the new dispatch."""
        arr = np.zeros((2, 3), dtype=np.uint8)
        img = _make_image(arr, ["y", "x"])
    ...
    def test_zarr_smoke_roundtrip_through_new_dispatch(self, tmp_path: Path) -> None:
        """End-to-end smoke: a .zarr round-trips through the new dispatch."""
        arr = np.arange(6, dtype=np.float32).reshape(2, 3)
        img = _make_image(arr, ["y", "x"])
        p = tmp_path / "smoke.zarr"
        SaveImage().save(Collection(items=[img], item_type=Image), BlockConfig(params={"path": str(p)}))
        result = LoadImage().load(BlockConfig(params={"path": str(p)}))
        assert isinstance(result, Collection)
        assert len(result) == 1
